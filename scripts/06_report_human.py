...
<p>The cracked Sniffles2 per-read caller (candidate → <b>CIGAR + split-read (SA) leadprov + split-and-remap</b> → unmodified
<code>sv.classify_splits</code>, no clustering) run <b>genome-wide</b> on merged BLS0005+BLS0006 sperm HiFi, haplotype-split
(strict-90) MAT + PAT, each mapped to its own HG002 assembly. Split-and-remap (out-of-phase satellite events) is run on the
alpha-satellite centromeres, re-mapping fragments with <b>minimap2</b> against a prebuilt index (winnowmap re-indexes the 6 Gb
genome per call, so minimap2 is used here for the human remap).</p>
